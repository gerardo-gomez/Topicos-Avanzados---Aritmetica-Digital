...
// tb_fp_adder
//
// Testbench para un sumador/restador FP16 (IEEE-754 half precision).
//
// A diferencia del TB de enteros, el resultado golden NO se calcula en SV:
// se precalcula con Berkeley SoftFloat (ver gen_f16_vectors.c) y se vuelca a
// un archivo de texto. Este TB lee ese archivo, aplica los operandos al DUT
// y compara resultado + banderas IEEE.
//
// Formato de cada linea del archivo de vectores:
//     <op> <rm> <a> <b> <result> <flags>   (todos en hexadecimal)
//   op     : 0 = add, 1 = sub
...
module tb_fp_adder;
  ...
  initial begin
    ...
    int op_i, rm_i, a_i, b_i, res_i, flags_i;
    ...
    while (!$feof(fd)) begin
      code = $fscanf(fd, "%h %h %h %h %h %h",
                     op_i, rm_i, a_i, b_i, res_i, flags_i);
      if (code != 6) begin
      ...
      if (pass) begin
        ...
        $error({"Error iter %0d: op=%0b rm=%0d srca=0x%04h srcb=0x%04h | ",
