// Description: Radix-4 / modified Booth multiplier that adds partial products using a Wallace tree (CSA) but doesn't include the final CPA (Carry-Propagate Adder).
...
//localparam int WALLACE_NUM_OPERANDS_OUT = 2              // Wallace tree reduces to 2 operands
  parameter  int WALLACE_NUM_OPERANDS_OUT = 2              // DON'T CHANGE (localparam not supported by Quartus)
) (
  input  logic                               [SRC1_WIDTH-1:0]   srca,
  input  logic                               [SRC2_WIDTH-1:0]   srcb,
  input  logic                                                  is_signed,
  output logic [WALLACE_NUM_OPERANDS_OUT-1:0][RESULT_WIDTH-1:0] wallace_result
);
...
//  return (triplet inside {3'b100, 3'b101, 3'b110}); // -A, -2A
  ...
  assign wallace_result = wallace_operands_out;
